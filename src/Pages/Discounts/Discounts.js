...
const Discounts = ({ product }) => {
  const { _id, name, img, price, author, discount } = product;
  return (
    ...
    <div className='w-full md:w-1/3 lg:w-1/4 bg-white  flex flex-col my-8 mx-2 justify-between items-start rounded-lg shadow-lg shadow-gray-600 text-gray-700'>
      ...
      <div className='px-4 py-2 mt-0 w-full flex flex-col justify-between'>
        <h1 className='text-sm font-semibold tracking-wide my-1'>Discount: <span className='text-red-600'>{discount}%</span></h1>
        <h1 className='text-sm font-semibold tracking-wide my-1 line-through'>Price Per Piece: {price} Tk</h1>
        <h1 className='text-sm font-semibold tracking-wide my-1'>Discounted Price: <span className='text-red-700'>{parseInt(price) * (100 - discount) / 100} Tk</span></h1>
